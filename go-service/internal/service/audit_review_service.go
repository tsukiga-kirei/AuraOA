...
func (s *AuditExecuteService) GetStatsWithParams(c *gin.Context, params dto.AuditListParams) (map[string]int, error) {
	...
	s.visibleWorkbenchQuery(c).
		Where("updated_at >= ? AND process_type IN ?", startOfDay, configuredTypes).
...
func (s *AuditExecuteService) getSnapshotMapDirect(c *gin.Context, tenantID uuid.UUID, processIDs []string) (map[string]*model.AuditProcessSnapshot, error) {
	...
	return s.auditSnapshotRepo.GetVisibleWorkbenchMap(c, processIDs, userID)
}
...
func (s *AuditExecuteService) ListProcessesPaged(c *gin.Context, params dto.AuditListParams) (*dto.AuditProcessListResponse, error) {
	...
	auditMap, err := s.auditLogRepo.GetLatestPersonalResultMap(c, processIDs, userID)
	if err != nil {
...
func (s *AuditExecuteService) listAllCompletedProcesses(c *gin.Context, tenantID uuid.UUID, username string, adapter oa.OAAdapter, params dto.AuditListParams) ([]map[string]interface{}, error) {
	...
	dataQ := s.visibleWorkbenchQuery(c).
...
func (s *AuditExecuteService) listCompletedProcessesPaged(c *gin.Context, tenantID uuid.UUID, username string, adapter oa.OAAdapter, params dto.AuditListParams) (*dto.AuditProcessListResponse, error) {
	...
	// 先查总数（用于分页）
	countQ := s.visibleWorkbenchQuery(c)
	if len(todoProcessIDs) > 0 {
...
func buildAuditResultFromLog(log *model.AuditLog) (result map[string]interface{}) {
	defer func() {
		if result != nil {
			result["trigger_source"] = log.TriggerSource
			result["user_id"] = log.UserID.String()
			if model.IsEmbedTrigger(log.TriggerSource) {
				result["result_source"] = "embed"
			} else {
				result["result_source"] = "personal"
			}
		}
	}()
	switch log.Status {
