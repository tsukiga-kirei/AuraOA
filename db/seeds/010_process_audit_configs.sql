...
-- ============================================================
-- 系统提示词模板（全局，8 条记录）
-- 2 条系统提示词（不区分尺度）+ 6 条用户提示词（3 尺度 × 2 阶段）
-- ============================================================
INSERT INTO system_prompt_templates
    (id, prompt_key, prompt_type, phase, strictness, content, description)
VALUES
-- 系统提示词（推理阶段）
('d4000000-0000-0000-0000-000000000001', 'system_reasoning', 'system', 'reasoning', NULL,
'你是 OA 智能审核系统的深度推理引擎。你的任务是对 OA 流程表单数据进行全面的合规性分析。
...
'系统推理提示词：定义 AI 在推理阶段的角色和分析方法论'),

-- 系统提示词（提取阶段）
('d4000000-0000-0000-0000-000000000002', 'system_extraction', 'system', 'extraction', NULL,
'你是 OA 智能审核系统的结构化提取引擎。你的任务是将前一阶段的推理分析结果转化为标准化的 JSON 格式输出。
...
仅输出 JSON，不要包含其他文字。',
'系统提取提示词：定义 AI 在提取阶段的输出格式和结构化要求'),

-- 用户提示词 — 严格（推理阶段）
('d4000000-0000-0000-0000-000000000003', 'user_reasoning_strict', 'user', 'reasoning', 'strict',
...
-- 用户提示词 — 严格（提取阶段）
('d4000000-0000-0000-0000-000000000004', 'user_extraction_strict', 'user', 'extraction', 'strict',
...
-- 用户提示词 — 标准（推理阶段）
('d4000000-0000-0000-0000-000000000005', 'user_reasoning_standard', 'user', 'reasoning', 'standard',
...
-- 用户提示词 — 标准（提取阶段）
('d4000000-0000-0000-0000-000000000006', 'user_extraction_standard', 'user', 'extraction', 'standard',
...
-- 用户提示词 — 宽松（推理阶段）
('d4000000-0000-0000-0000-000000000007', 'user_reasoning_loose', 'user', 'reasoning', 'loose',
...
-- 用户提示词 — 宽松（提取阶段）
('d4000000-0000-0000-0000-000000000008', 'user_extraction_loose', 'user', 'extraction', 'loose',
...
(
    ...
    (SELECT jsonb_build_object(
        ...
        'system_reasoning_prompt', (SELECT content FROM system_prompt_templates WHERE prompt_key = 'system_reasoning'),
        'system_extraction_prompt', (SELECT content FROM system_prompt_templates WHERE prompt_key = 'system_extraction'),
