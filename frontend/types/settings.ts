...
export interface SystemGeneralConfig {
    ...
    /** attachment.document_parser_types — 使用代码解析正文的文件类型（逗号分隔） */
    attachment_document_parser_types: string
...
export function mapConfigItems(items: ConfigItem[]): Partial<SystemGeneralConfig> {
    ...
    return {
        ...
        ...(str('attachment.document_parser_types') !== undefined && { attachment_document_parser_types: kv['attachment.document_parser_types'] }),
...
export function configToUpdateRequest(cfg: SystemGeneralConfig): ConfigUpdateRequest {
    return {
        ...
        'attachment.document_parser_types': cfg.attachment_document_parser_types ?? '',
