...
const targetWorkflowPickerOpen = ref(false)
...
const loadWorkflowTargetFields = async (target?: ProcessInfo) => {
  const mount = workflowMount.value
  ...
  try {
    ...
    else message.success('目标流程字段已加载')
  } catch (e: any) {
...
const setWorkflowTargetMode = (specified: boolean) => {
  ...
  if (specified) {
    ...
    if (!mount.workflow.target_process_type && !mount.workflow.target_workflow_id) {
      openTargetWorkflowPicker()
    }
  } else {
...
const openTargetWorkflowPicker = () => {
  const mount = workflowMount.value
  targetWorkflowKeyword.value = mount?.workflow?.target_process_type || ''
  selectedTargetWorkflowID.value = mount?.workflow?.target_workflow_id || ''
  targetWorkflowPickerOpen.value = true
  searchTargetWorkflows()
}
...
const searchTargetWorkflows = async () => {
  ...
  try {
    targetWorkflowRows.value = await authFetch<ProcessInfo[]>(props.workflowSearchEndpoint, {
      method: 'POST',
      body: { keyword: targetWorkflowKeyword.value.trim() },
    })
    if (!selectedTargetWorkflowID.value && targetWorkflowRows.value.length) {
      selectedTargetWorkflowID.value = targetWorkflowRows.value[0].workflow_id || targetWorkflowRows.value[0].process_name
    }
  } catch (e: any) {
...
const confirmTargetWorkflow = async () => {
  const row = targetWorkflowRows.value.find(item => (item.workflow_id || item.process_name) === selectedTargetWorkflowID.value)
  const mount = workflowMount.value
  if (!row || !mount?.workflow) {
  ...
  mount.workflow.data_mode = 'selected_fields'
  targetWorkflowPickerOpen.value = false
  await loadWorkflowTargetFields(row)
}
...
<template>
    ...
    <a-form layout="vertical" style="margin-top: 16px;">
      ...
      <div v-if="workflowMount" class="context-panel">
        ...
        <a-row :gutter="12">
          ...
          <a-col :span="12">
            <a-form-item label="流程基础信息">
              <a-checkbox-group v-model:value="workflowMount.workflow!.basic_fields" :options="workflowBasicOptions" />
            </a-form-item>
          </a-col>
        </a-row>
        ...
        <div v-if="workflowMount.workflow?.data_mode === 'selected_fields'" class="target-flow-box">
          <div class="target-flow-summary">
            <div>
              <div class="target-flow-label">目标流程</div>
              <div class="target-flow-value">{{ targetWorkflowSummary || '尚未选择' }}</div>
            </div>
            <a-button :loading="loadingWorkflowFields" @click="openTargetWorkflowPicker">
              {{ targetWorkflowSummary ? '更换流程' : '选择流程' }}
            </a-button>
          </div>
  ...
  <a-modal
    v-model:open="targetWorkflowPickerOpen"
    title="选择目标流程"
    :width="720"
    ok-text="引入流程"
    cancel-text="取消"
    :confirm-loading="loadingWorkflowFields"
    @ok="confirmTargetWorkflow"
  >
      ...
      <a-spin :spinning="searchingWorkflows">
        <div class="workflow-result-list">
          <button
            ...
            :class="{ active: selectedTargetWorkflowID === (item.workflow_id || item.process_name) }"
            @click="selectedTargetWorkflowID = item.workflow_id || item.process_name"
          >
          ...
          <a-empty v-if="!targetWorkflowRows.length && !searchingWorkflows" description="未找到匹配流程" />
        </div>
      </a-spin>
    </div>
  </a-modal>
