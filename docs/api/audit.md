...
分页查询 OA 待审批流程列表，支持多维度筛选。当前待办会合并个人工作台审核与 OA 嵌入审核结果；同一流程优先显示当前用户的个人审核结果，没有个人结果时显示 OA 嵌入结果。租户管理员的历史列表还会纳入有权限流程的 OA 嵌入结果。`audit_result.result_source` 为 `personal` 或 `embed`。
