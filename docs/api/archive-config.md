...
配置保存不会自动重建既有归档复盘结果。每个流程首次复盘时锁定最终生效配置版本，普通再次复盘
继续使用原版本；只有用户明确选择使用最新配置时才升级流程绑定。
...
- `import-preview`：使用 `multipart/form-data` 上传 `config_id` 与 `file`，先按文件类型和管理员选择路由到内置解析、代码文档解析服务或 MinerU，再由统一 AI 调用入口返回可编辑草稿，不写库。
