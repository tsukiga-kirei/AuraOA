...
在对外接口（如审核/复盘/总结链路与嵌入页面上下文）中，`config_version_no` 统一映射展示为对应的租户基础版本号（即管理端规则配置中展示的当前可用版本号，如 v10），保持业务人员心智模型一致；底层不可变执行快照的精确溯源与审计继续由 `config_version_id` UUID 严格保障。
