...
服务端先按文件类型和管理员选择路由到内置解析、代码文档解析服务或 MinerU，再通过 `AIModelCallerService` 生成结构化规则草稿。此接口**不写入规则库**。每条草稿包含 `rule_content`、`rule_scope`、`related_flow`、`context_recommended`、`confidence`、`reasoning`。
