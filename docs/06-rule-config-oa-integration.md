...
| AI 模型调用体系 | Go 层按部署类型封装调用逻辑，支持本地 Xinference 和云端阿里百炼 |
...
| oa_type | 适配器 | 说明 |
|---------|--------|------|
| `weaver_e9` | `Ecology9Adapter` | 泛微 Ecology9 |

扩展新 OA 类型只需：(1) 实现 `OAAdapter` 接口，(2) 在工厂函数中注册 case。
...
#### XinferenceCaller（本地部署）

位置：`go-service/internal/pkg/ai/xinference.go`

通过 OpenAI 兼容 API 调用本地 Xinference 部署的模型。

| 配置项 | 说明 |
|--------|------|
| Endpoint | `http://<host>:<port>/v1/chat/completions` |
| 认证方式 | 无需 API Key（本地部署） |
| 协议 | OpenAI Chat Completions API 兼容 |

#### AliyunBailianCaller（云端部署）

位置：`go-service/internal/pkg/ai/aliyun_bailian.go`
...
| deploy_type | 调用器 | 说明 |
|-------------|--------|------|
| `local` | `XinferenceCaller` | 本地 Xinference 部署 |
| `cloud` | `AliyunBailianCaller` | 阿里云百炼云端 |
...
| 50303 | `ErrAIDeployTypeUnsupported` | 503 | 不支持的 AI 部署类型 |
...
| AI Caller | `internal/pkg/ai/xinference.go` | Xinference 调用器 |
| AI Caller | `internal/pkg/ai/aliyun_bailian.go` | 阿里百炼调用器 |
| AI Caller | `internal/pkg/ai/factory.go` | AI 调用器工厂 |
