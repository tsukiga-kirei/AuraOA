...
| 激活角色 | 看到的菜单 | 数据范围 |
|----------|-----------|----------|
| 示例集团总部 · 业务用户 | 仪表盘 + 业务角色 `page_permissions` 并集对应的菜单 + 个人设置 | 示例集团总部的数据 |
| 华东分公司 · 业务用户 | 同上（按该租户下业务角色权限过滤） | 华东分公司的数据 |
| 示例集团总部 · 租户管理员 | 仪表盘 + 租户管理菜单（按用户业务角色 `page_permissions` 并集过滤，无业务角色时全量展示） | 示例集团总部的管理数据 |
...
| 角色类型 | 展示的菜单分组 |
|----------|---------------|
| `business` | 仪表盘 + 业务菜单（由业务角色 `page_permissions` 并集过滤，仅显示有权限的项）+ 个人设置 |
| `tenant_admin` | 仪表盘 + 租户管理菜单（若用户同时拥有业务角色，则按 `page_permissions` 并集过滤；若无业务角色则全量展示）|
| `system_admin` | 仪表盘 + 系统管理菜单（全局监控、租户管理、系统设置）|

> **注意**：`business` 角色的业务菜单不再全量展示，而是根据用户在当前租户下所有业务角色的 `page_permissions` 并集进行过滤。如果并集为空，则不显示业务菜单分组。`tenant_admin` 角色的租户管理菜单同样受此过滤影响：当用户拥有业务角色的 `page_permissions` 时，仅展示权限并集中包含的管理菜单项；若用户无业务角色（`page_permissions` 为空），则全量展示所有租户管理菜单。
...
- [业务角色说明](./BUSINESS_ROLES.md) — 租户级业务角色（业务用户、审计管理员、租户管理员）的定义、页面权限、依赖关系
- [系统角色与业务角色的关联关系](./ROLE_RELATIONSHIP.md) — 两层角色体系的联动机制、页面可见性规则、数据文件对照
