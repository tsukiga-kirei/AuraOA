...
以用户最新布局要求为准：全局侧栏按智能体分组会话，可折叠、搜索及组内新建；聊天页不设第二个历史侧栏。回复使用文档式 GFM Markdown。系统工具、Skill 和 MCP 全部默认收起为紧凑执行行，显示来源、名称、状态；展开后才展示结果和查询条件，长输出限制高度。旧文档的大卡片示意不再作为默认展示标准。
